...
def encrypt_file(filename, cmk_id):
    ...
    print("length=",len(data_key_encrypted));
...
def main():
    ...
    if file_to_encrypt:
        ...
        if encrypt_file(file_to_encrypt, cmk_arn):
            logging.info('document.pdf encrypted to ','document.pdf.encrypted')

            # Decrypt the file
            if decrypt_file(file_to_encrypt):
                # Decrypted file contents are written to <file_to_encrypt>.decrypted
                logging.info('document.pdf.encrypted decrypted to ','document.pdf.decrypted')
